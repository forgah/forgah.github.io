...
function list(itemsArray, type = 'movies') {
    return new Promise((resolve) => {
        ...
        genreOptions.unshift('<option value="">اختار النوع</option>');
        ...
        const generateItemCards = (items) => {
            return items.map(item => {
                ...
                // Add the tip attribute if the tip parameter exists
                const tipAttribute = item.tip ? `tip="${item.tip}"` : '';

                return `
                    <div class="item-card" ${tipAttribute} data-genres="${item.genre ? item.genre.join(',') : ''}">
        ...
        Swal.fire({
            ...
            cancelButtonText: 'اقفل', // Set the cancel button text to "اقفل"
            width: '90%',
            customClass: {
...
// Function to automatically parse vocabulary from subtitles
async function AutoVocab(url) {
    try {
        ...
        if (matches.length === 0) {
            console.log("No vocabulary pairs found in the subtitles.");
        }
...
async function vocab() {
    removeCachedMovie();
    let cachedVocab = JSON.parse(localStorage.getItem('cachedVocab')) || [];

    // New Mode: If cachedVocab is true (boolean or string) and cachedSubtitles is available
    if ((cachedVocab === true || cachedVocab === "true") && cachedSubtitles) {
        console.log("AutoVocab mode activated. Fetching subtitles...");
        // Automatically parse vocabulary from subtitles
        const parsedVocab = await AutoVocab(cachedSubtitles);

        if (parsedVocab.length > 0) {
            // Use the parsed vocabulary for the existing mode
            cachedVocab = parsedVocab; // Replace cachedVocab with parsedVocab
            localStorage.setItem('cachedVocab', JSON.stringify(cachedVocab));
        } else {
            console.log("No vocabulary pairs found in the subtitles.");
            Swal.fire('No Vocabulary Found', 'No vocabulary pairs were found in the subtitles.', 'info');
            return;
        }
    }

    // Existing Mode: cachedVocab is an array of arrays
    if (!Array.isArray(cachedVocab) || cachedVocab.length === 0) {
    ...
    const showVocab = (index) => {
        const [word, translation] = cachedVocab[index]; // Destructure the word and translation
        Swal.fire({
...
function patternize(targetElementSelector, options = {}) {
  ...
  patternize('.lighting', {
    images: [
  "https://i.pinimg.com/736x/89/a5/dc/89a5dc914eb2161b705fadd83bbad0e6.jpg"
    ],
    height: '105%',
    size: { width: 200, height: 355 },
    rotation: 0,
  ...
  patternize('.floor', {
    images: [
   "https://i.pinimg.com/736x/69/a0/19/69a0193eca0226aa3210aee371e536ba.jpg"
 ],
    height: '310%',
    rotation: 0,
    gapX: 0,
    gapY: 0,
    size: { width: 450, height: 580 },
    animate: false, // Disable animation
  });
