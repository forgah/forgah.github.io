...
class RadialMenu {
  ...
  injectStyles() {
    ...
    const styles = `
      ...
      .radial-item:hover .radial-title {
        opacity: 1;
  ...
  handleClickOutside(event) {
    // If clicking outside the radial menu
    if (this.activeWheel && !this.activeWheel.contains(event.target)) {
      this.close();
    }
  }
  ...
  open(items) {
    ...
    wheel.className = 'radial-menu';
    wheel.style.left = `${e_mousemove[0]}px`;
    wheel.style.top = `${e_mousemove[1]}px`;
    ...
    items.forEach((item, index) => {
      ...
      Object.assign(element.style, {
        ...
        background: `linear-gradient(145deg, ${item.color}, ${item.gradientColor || item.color})`,
        width: `${item.size || 70}px`,
        height: `${item.size || 70}px`,
      });
  ...
  close() {
    if (!this.activeWheel) return;
    
    // Remove click outside listener
    document.removeEventListener('click', this.handleClickOutside);
    
    const items = this.activeWheel.querySelectorAll('.radial-item');
    items.forEach((item, index) => {
      setTimeout(() => {
    ...
    setTimeout(() => {
      this.activeWheel.remove();
      this.activeWheel = null;
      this.isOpen = false;
    }, items.length * 30 + 300);
  }

  // Add cleanup method to ensure proper removal of event listeners
  destroy() {
    if (this.isOpen) {
      this.close();
    }
